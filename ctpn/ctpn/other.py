...
def draw_boxes(im,
    ...
    im = im.copy()
    index = 0
    #cv2.imwrite('C:/Users/50298/Desktop/ocr/beforLine.jpg',im)
    for box in bboxes:
        if color == None:
        ...
        # cv2.waitKey(0)
        # cv2.imshow('kk', im)
        point_list.append([[int(x1), int(y1)], [int(x2), int(y2)], [int(x3), int(y3)], [int(x4), int(y4)]])

        #cv2.imwrite('D:/ocr/CHINESE-OCR/test/vertical_res.jpg',im)
    print(point_list)
    return text_recs, im
